...
> user driving one or a handful of agents (e.g. a coding CLI). It has no auth,
> no per-user quota and no metrics; `DS_MAX_CONCURRENT` is a self-protection
...
### The same conversation over the Responses API
...
Returns a JSON liveness/readiness report: `status` (`ok` / `shutting_down`),
`uptime_ms`, `remote_host`, an `accounts` summary (`total` / `usable` /
`available` / `cooling`), `sessions`, `shutting_down`, and - while the server
is up - a `concurrency` block (`in_flight` / `limit` / `available`).
...
- **Streaming.** Server-Sent Events: `response.created`,
  `response.output_item.added`, `response.output_text.delta`,
  `response.function_call_arguments.delta`, `response.completed` (or
  `response.incomplete`), then `data: [DONE]`.
...
| `DS_AUTH_DIR` | `.auth` | Directory scanned for `*.json` auth configs (sorted). Audited at startup (unset/missing dir, no `*.json` files, or none loading all produce a clear log line). |
...
```
index.js                  thin HTTP router + concurrency semaphore
lib/
